...
public class WanderLust {
    ...
    static class Destination {
        String id, name, icon, region, state;
        String[] tags, features;
        int price, seats, maxSeats;
        String duration, people, weather, temp, desc;
        Destination(String id, String name, String icon, String region, String state,
            ...
            this.id = id; this.name = name; this.icon = icon; this.region = region;
    ...
    static String[][] weatherData = {
        {"Kerala, India",       "28C", "Humid, partly cloudy"},
        {"Ooty, India",         "18C", "Cool, misty mornings"},
        {"Manali, India",       "-2C", "Snowfall expected"},
        {"Agra, India",         "24C", "Clear skies"},
        {"Jaipur, India",       "26C", "Dry, sunny"},
        {"Andaman, India",      "30C", "Tropical, warm seas"},
        {"Ladakh, India",       " 5C", "High altitude winds"},
        {"Darjeeling, India",   "12C", "Foggy, scenic"},
        {"Goa, India",          "32C", "Beach weather perfect"},
        {"Paris, France",       "14C", "Partly cloudy, mild"},
        {"New York, USA",       " 8C", "Chilly, clear skies"},
        {"Tokyo, Japan",        "16C", "Spring bloom season"},
        {"Dubai, UAE",          "35C", "Hot & dry, sunny"},
        {"Nairobi, Kenya",      "22C", "Safari weather, clear"},
        {"Rome, Italy",         "19C", "Pleasant, light breeze"},
        {"Bali, Indonesia",     "29C", "Warm, tropical showers"},
        {"Sydney, Australia",   "23C", "Sunny, sea breeze"},
        {"Zurich, Switzerland", " 6C", "Cold, snow on peaks"},
        {"London, UK",          "11C", "Overcast, light rain"},
        {"Cancun, Mexico",      "31C", "Hot, crystal clear sea"},
        {"Bangkok, Thailand",   "33C", "Hot & humid, hazy"},
        {"Cape Town, S.Africa", "20C", "Mild, breezy coast"},
        {"Barcelona, Spain",    "18C", "Warm, mostly sunny"},
    };

    // -- Destinations data ---------------------------------------
    static Destination[] destinations = {
        new Destination("kerala", "Kerala", "[KL]", "south", "God's Own Country",
            new String[]{"Houseboat", "Wildlife", "Culture"},
            new String[]{"Houseboat Stay", "Ayurvedic Spa", "Spice Tour"},
            18500, 12, 20, "6 Days", "2-8", "Humid, partly cloudy", "28C",
            "Drift through emerald backwaters on a houseboat, explore spice plantations and witness Kathakali at sunset."),
        new Destination("ooty", "Ooty", "[OT]", "south", "Nilgiris, Tamil Nadu",
            new String[]{"Hill Station", "Nature", "Tea Garden"},
            new String[]{"Toy Train Ride", "Tea Factory", "Botanical Garden"},
            12800, 8, 15, "4 Days", "2-6", "Cool, misty mornings", "18C",
            "Ride the iconic toy train through rolling tea estates, wake up to cool mountain mist in the Blue Mountains."),
        new Destination("manali", "Manali", "[MN]", "north", "Himachal Pradesh",
            new String[]{"Snow", "Adventure", "Trekking"},
            new String[]{"River Rafting", "Snow Activities", "Rohtang Pass"},
            22000, 0, 12, "7 Days", "2-10", "Snowfall expected", "-2C",
            "Conquer the Rohtang Pass, camp under stars and feel the rush of river rafting in the icy Beas."),
        new Destination("agra", "Agra", "[AG]", "north", "Uttar Pradesh",
            new String[]{"Heritage", "History", "Architecture"},
            new String[]{"Taj Mahal Sunrise", "Agra Fort", "Fatehpur Sikri"},
            9500, 5, 20, "3 Days", "2-8", "Clear skies", "24C",
            "Stand in awe before the Taj Mahal at sunrise, wander Agra Fort's labyrinthine halls."),
        new Destination("jaipur", "Jaipur", "[JP]", "north", "Rajasthan",
            new String[]{"Royalty", "Culture", "Shopping"},
            new String[]{"Elephant Ride", "City Palace", "Camel Safari"},
            13500, 18, 25, "4 Days", "2-12", "Dry, sunny", "26C",
            "Ride an elephant to Amber Fort, browse dazzling bazaars and watch sunset over Nahargarh's ramparts."),
        new Destination("andaman", "Andaman", "[AN]", "islands", "Islands",
            new String[]{"Beach", "Scuba Diving", "Islands"},
            new String[]{"Scuba Diving", "Glass Bottom Boat", "Ross Island"},
            28000, 0, 10, "6 Days", "2-6", "Tropical, warm seas", "30C",
            "Snorkel through world-class coral reefs, laze on powdery white beaches and explore the Cellular Jail."),
        new Destination("ladakh", "Ladakh", "[LD]", "north", "Jammu & Kashmir",
            new String[]{"High Altitude", "Monasteries", "Offbeat"},
            new String[]{"Pangong Lake Camp", "Monastery Tour", "Zanskar Valley"},
            32000, 4, 10, "8 Days", "2-8", "High altitude winds", "5C",
            "Traverse moonscapes at 17,000 ft, meditate in ancient cliff monasteries and camp beside Pangong Tso."),
        new Destination("darjeeling", "Darjeeling", "[DJ]", "east", "West Bengal",
            new String[]{"Tea", "Mountain View", "Heritage"},
            new String[]{"Tea Plantation Walk", "Toy Train", "Tiger Hill Sunrise"},
            15500, 10, 16, "5 Days", "2-6", "Foggy, scenic", "12C",
            "Sip first-flush Darjeeling tea while Kangchenjunga glows at dawn, ride the beloved toy train."),
        new Destination("goa", "Goa", "[GA]", "west", "Goa",
            new String[]{"Beach", "Nightlife", "Culture"},
            new String[]{"Beach Shacks", "Water Sports", "Old Goa Churches"},
            14000, 3, 20, "5 Days", "2-10", "Beach weather perfect", "32C",
    ...
    static void success(String msg) { System.out.println(GREEN  + "  [OK]  " + msg + RESET); }
    static void error(String msg)   { System.out.println(RED    + "  [!!]  " + msg + RESET); }
    static void info(String msg)    { System.out.println(CYAN   + "  [i]   " + msg + RESET); }
    ...
    static void printRupees(int amount) {
        System.out.printf("%sRs.%,d%s", GOLD, amount, RESET);
    }
    ...
    static void splashScreen() {
        ...
        System.out.println();
        System.out.println(CYAN + "       9+ Destinations  -  2K+ Travellers  -  Rated 4.9" + RESET);
        System.out.println();
    ...
    static void userMenu() {
        while (true) {
            ...
            System.out.println("  " + BOLD + "2." + RESET + "  Destinations");
            System.out.println("  " + BOLD + "3." + RESET + "  Packages");
            System.out.println("  " + BOLD + "4." + RESET + "  My Wishlist  (" + wishlist.size() + " saved)");
            System.out.println("  " + BOLD + "5." + RESET + "  My Profile");
            System.out.println("  " + BOLD + "6." + RESET + "  Weather Ticker");
    ...
    static void homePage() {
        ...
        System.out.println();
        String[][] features = {
            {"[>]", "Curated Packages",  "Handpicked itineraries by expert travel designers"},
            {"[$]", "Secure Payments",   "UPI, Net Banking, Cards - zero hidden fees"},
            {"[H]", "Premium Stays",     "Boutique hotels balancing comfort and character"},
            {"[R]", "Real-Time Updates", "Live seat availability & instant confirmations"},
            {"[M]", "9 Destinations",    "Kerala, Ooty, Manali, Agra, Jaipur, Andaman, Ladakh, Darjeeling, Goa"},
            {"[L]", "Flexible Booking",  "Easy cancellations, date changes, group discounts"},
        };
        ...
        for (Destination d : destinations) {
            System.out.printf("  %s %s%-12s%s %sRs.%,d%s/person%n",
                d.icon, BOLD, d.name, RESET, GOLD, d.price, RESET);
        }
        System.out.println();
    ...
    static void destinationsPage() {
        while (true) {
            ...
            System.out.println("  " + BOLD + "5." + RESET + "  Weather Ticker");
    ...
    static void browseDestinations(List<Destination> list) {
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            System.out.printf("  %s%d.%s %s %s%-12s%s  %sRs.%,d%s  %s  %s%n",
                BOLD, i + 1, RESET,
                d.icon, BOLD, d.name, RESET,
                GOLD, d.price, RESET,
                d.seatsLabel(),
    ...
    static void destDetailView(Destination d) {
        header("DESTINATION - " + d.icon + " " + d.name);
        System.out.println();
        ...
        section("Details");
        System.out.printf("  %-18s %sRs.%,d/person%s%n", "Price:",    GOLD, d.price, RESET);
        ...
        section("Highlights");
        System.out.println();
        for (String f : d.features) System.out.println("  " + GREEN + "[OK]" + RESET + "  " + f);
    ...
    static void searchDestination() {
        ...
        if (known.isPresent()) {
            ...
            System.out.printf("  |  India - Custom Destination                               |%n");
            System.out.printf("  |  Tags: Sightseeing - Culture - Photography                |%n");
            System.out.printf("  |  Price: %sRs.%,d/person%s                                     |%n", GOLD, price, RESET);
            System.out.printf("  |  Seats: %-47s|%n", seatLabel + "   ");
            ...
            if (ch == 1) {
                Destination custom = new Destination(
                    "custom-" + query, cap, "[C]", "custom", "India",
    ...
    static void filterByType() {
        ...
        String[][] types = {
            ...
            {"heritage", "Heritage destinations"},
    ...
    static void filterByRegion() {
        ...
        String[] labels  = {"North India", "South India", "East India", "West India", "Islands"};
        for (int i = 0; i < labels.length; i++)
            System.out.println("  " + BOLD + (i + 1) + "." + RESET + "  " + labels[i]);
    ...
    static void toggleWishlist(Destination d) {
        if (wishlist.contains(d.id)) {
            ...
            wishlist.add(d.id);
            success("Added " + d.name + " to your wishlist!");
        }
        shortPause();
    }
    ...
    static void packageGrid(List<Destination> list) {
        ...
        for (int i = 0; i < list.size(); i++) {
            Destination d = list.get(i);
            String priceMMT = String.format("Rs.%,d", (int)(d.price * 1.09));
            String priceCT  = String.format("Rs.%,d", (int)(d.price * 1.06));
            System.out.println("  " + GOLD + "-".repeat(56) + RESET);
            System.out.printf("  %s%d.%s %s %s%-14s%s  %s-%s %s%n",
                BOLD, i + 1, RESET, d.icon, BOLD, d.name + " Package", RESET, CYAN, RESET, d.duration);
            System.out.printf("     Price: %sRs.%,d%s/person    MMT: %s%s%s  ClearTrip: %s%s%s%n",
                GOLD, d.price, RESET, DIM, priceMMT, RESET, DIM, priceCT, RESET);
            System.out.printf("     Seats: %-30s  Group: %s%n", d.seatsLabel(), d.people);
            System.out.printf("     Features: %s%s%s%n", DIM, String.join(" - ", d.features), RESET);
        }
        System.out.println("  " + GOLD + "-".repeat(56) + RESET);
        System.out.println("  " + BOLD + (list.size() + 1) + "." + RESET + "  Back");
        System.out.println();
    ...
    static void bookingFlow(Destination d, Object unused) {
        ...
        System.out.println();
        loadingBar("  Processing payment of Rs." + String.format("%,d", total));
        System.out.println();
        ...
        System.out.printf("  |  Destination: %-35s|%n", d.icon + " " + d.name);
        System.out.printf("  |  Package:     %-35s|%n", d.duration + " Tour");
        System.out.printf("  |  Travellers:  %-35s|%n", trav + " person" + (trav > 1 ? "s" : ""));
        System.out.printf("  |  Amount Paid: %-35s|%n", "Rs." + String.format("%,d", total) + " via " + method.split(" ")[0]);
    ...
    static void wishlistPage() {
        ...
        System.out.println();
        if (wishlist.isEmpty()) {
            warn("Your wishlist is empty. Add a destination to save it here.");
            System.out.println();
            input("Press Enter");
            return;
        }
        ...
        for (int i = 0; i < saved.size(); i++) {
            ...
            System.out.printf("  %s%d.%s %s %s%-14s%s  %sRs.%,d%s  %s%n",
                BOLD, i + 1, RESET, d.icon, BOLD, d.name, RESET, GOLD, d.price, RESET, d.seatsLabel());
        }
        System.out.println();
    ...
    static void profilePage() {
        while (true) {
            ...
            System.out.printf("  %-20s %sGold Member%s%n", "Status:", GOLD, RESET);
            System.out.println();
    ...
    static void myBookings() {
        ...
        if (mine.isEmpty()) {
            ...
            for (int i = 0; i < mine.size(); i++) {
                ...
                System.out.printf("  %s%d.%s  %-25s  Ref: %s%s%s%n",
                    BOLD, i + 1, RESET, b.destName, CYAN, b.ref, RESET);
                System.out.printf("      Package: %-30s%n", b.pkg);
                System.out.printf("      Paid: %sRs.%,d%s  -  Travellers: %d  -  via %s%n",
    ...
    static void notifications() {
        ...
        if (currentUser.trips > 0) {
            System.out.println(CYAN + "  [T]   " + BOLD + "You have " + currentUser.trips + " completed trip(s)!" + RESET);
    ...
    static void adminMenu() {
        while (true) {
            ...
            System.out.println("  " + BOLD + "6."  + RESET + "  Destinations & Seat Availability");
            System.out.println("  " + BOLD + "7."  + RESET + "  User Login / Logout History");
            System.out.println("  " + BOLD + "8."  + RESET + "  Global Weather");
            System.out.println("  " + BOLD + "9."  + RESET + "  Notices & Quick Actions");
            System.out.println("  " + BOLD + "10." + RESET + " Platform Information");
    ...
    static void adminStats() {
        ...
        System.out.printf("  %s|%s  %-20s  %s%s%-8s%s                   %s|%s%n",
            GOLD, RESET, "Total Revenue",       BOLD, GREEN, "Rs." + String.format("%,d", totalRevenue), RESET, GOLD, RESET);
        System.out.printf("  %s|%s  %-20s  %s%s%-8s%s                   %s|%s%n",
        ...
        if (allBookings.isEmpty()) {
            ...
            for (Booking b : allBookings) {
                System.out.printf("  %-10s %-18s %-24s %s%-10s%s %s%n",
                    ...
                    GOLD, "Rs." + String.format("%,d", b.amount), RESET, DIM + b.time + RESET);
            }
            line();
            int total = allBookings.stream().mapToInt(b -> b.amount).sum();
            System.out.println();
            System.out.println("  Total Revenue: " + GOLD + BOLD + "Rs." + String.format("%,d", total) + RESET);
        }
        System.out.println();
        input("Press Enter to return");
    }
    ...
    static void adminDestinations() {
        ...
        for (Destination d : destinations) {
            ...
            System.out.printf("  %s %-10s  %sRs.%,d%s  %-14s  %s  %sRs.%,d%s%n",
                d.icon, d.name, GOLD, d.price, RESET,
    ...
    static void adminUserLog() {
        ...
        if (userLog.isEmpty()) {
            ...
            for (SessionLog s : userLog) {
                String status = s.isOnline ? GREEN + "Online" + RESET : DIM + "Offline" + RESET;
    ...
    static void adminNotices() {
        ...
        System.out.println(GREEN + "  [OK] Jaipur has the highest availability (18 seats open)" + RESET);
        ...
        System.out.println(CYAN + "  [i]  3 destinations with LOW availability:" + RESET);
        ...
        for (Destination d : destinations) {
            if (d.seats > 0) {
                System.out.printf("     %-12s  %sRs.%,d%s%n", d.name, GOLD, d.seats * d.price, RESET);
            }
        }
        System.out.println();
        input("Press Enter to return");
